...
class TestConfig(unittest.TestCase):
                ...
                "rules": [
                    {
                        ...
                        "query": "project = LPTOCPCI AND resolution = Unresolved AND Issuetype = bug",
                    },
                    {
    ...
    def test_config_initialization_with_valid_config(
        self,
        mock_get_logger,
        mock_jira,
        mock_database,
        ...
        config = Config(
            filepath=Path(self.temp_file.name),
            check_team=False,
            init_db=True,
            local=True,
        )
    ...
    @patch("cli.obj.config.Jira")
    @patch("cli.obj.config.Database")
    def test_config_initialization_without_filepath(
        self,
        mock_get_logger,
        mock_jira,
        mock_database,
    ...
    @patch("cli.obj.config.get_logger")
    def test_config_initialization_without_filepath_and_env_variable(
        self,
        mock_get_logger,
    ):
        mock_get_logger.return_value = MagicMock()
    ...
    @patch("cli.obj.config.Jira")
    @patch("cli.obj.config.Database")
    def test_config_initialization_with_invalid_jira_auth(
        self,
        mock_database,
        mock_jira,
        mock_get_logger,
    ...
    @patch("cli.obj.config.Jira")
    @patch("cli.obj.config.Database")
    def test_config_initialization_with_invalid_database_auth(
        self,
        mock_database,
        mock_jira,
        mock_get_logger,
